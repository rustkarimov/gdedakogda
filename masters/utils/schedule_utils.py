...
class ScheduleCalculator:
    ...
    def get_actual_slot_duration(self, service_duration: int) -> int:
        """Возвращает реальную длительность слота с учётом профиля мастера"""
        if self.master.profile_type == 'travel':
            return service_duration + self.master.travel_time
    ...
    def get_available_dates(self, days_ahead: int = 60, min_service_duration: int = 30) -> List[date]:
        ...
        for i in range(days_ahead):
            ...
            # Используем актуальную длительность с учётом профиля мастера
            actual_duration = self.get_actual_slot_duration(min_service_duration)
    ...
    def generate_time_slots_for_multiple_services(self, target_date: date, services_durations: List[int], slot_step: int = 15, exclude_booking_ids: List[int] = None, current_time: time = None):
        ...
        # Учитываем профиль мастера
        actual_duration = self.get_actual_slot_duration(total_duration)
